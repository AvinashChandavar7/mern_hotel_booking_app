    ...
    <div className="flex items-center justify-center">
      <ul className="flex items-center justify-center w-full gap-2 rounded-md border-slate-300">
        {pageNumbers.map((number, index) => (
          <li key={index} className={`px-4 py-2 cursor-pointer font-bold rounded-md border border-blue-500 ${page === number ? "bg-blue-500 text-white" : ""}`}>
            <button onClick={() => memoizedOnPageChange(number)}>
              {number}
            </button>
          </li>
        ))}
